...
def transition_to_next_period(v: np.ndarray) -> np.ndarray:
    ...
    for index in range(TOTAL_SIZE - 1):
        for prod_time in range(2, MAX_PROD_TIME):
            cost_not_to_failure = PROB_MATRIX_1[:NUM_STATES, :NUM_STATES].dot(
                    v[:NUM_STATES, PROD_1, prod_time - 1, 0, index])
            cost_to_failure = PROB_MATRIX_1[:NUM_STATES, NUM_STATES].dot(
        ...
        cost_not_to_failure = PROB_MATRIX_2[:NUM_STATES, :NUM_STATES].dot(
                v[:NUM_STATES, 0, 0, 0, index + 1])
        cost_to_failure = PROB_MATRIX_2[:NUM_STATES, NUM_STATES].dot(
                v[NUM_STATES, 0, 0, 0, index])
